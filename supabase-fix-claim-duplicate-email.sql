...
-- Keep this file safe if run manually: include expiry check and idempotent completion behavior.
...
BEGIN
  ...
  WHERE completion_token = trim(p_token) AND status = 'approved'
    AND (token_expires_at IS NULL OR token_expires_at > NOW());
  IF NOT FOUND THEN
    SELECT * INTO v_request
    FROM registration_requests
    WHERE completion_token = trim(p_token) AND status = 'completed';
  ...
  IF FOUND THEN
    ...
    RETURN QUERY SELECT v_existing_student.pin_number, v_existing_student.name, v_existing_student.email;
    RETURN;
  END IF;
